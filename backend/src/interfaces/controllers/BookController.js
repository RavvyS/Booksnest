...
exports.getBookById = async (req, res) => {
  try {
    ...
    if (req.user) {
      const activeBorrow = await borrowRepository.findActiveBorrow(
        req.user.id,
        req.params.bookId,
      );
...
exports.searchExternal = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "External search failed", error: error.message });
  }
};
...
exports.getFreeExternalBooks = async (req, res) => {
  try {
    ...
    const results = await bookSearchService.searchFreeBooks(
      subject || "fiction",
    );
    res.json(results);
  } catch (error) {
    res
      .status(500)
      .json({ message: "Failed to fetch free books", error: error.message });
  }
};
